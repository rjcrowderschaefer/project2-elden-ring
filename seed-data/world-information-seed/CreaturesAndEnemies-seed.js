const CreaturesAndEnemies = [
    {
        name: 'Crucible Knight',
        img: 'data:image/jpeg;base64,',
        description: 'Crucible Knight is a Field Boss in Elden Ring. These heavily-armored warriors take patience and practice to defeat. This is an optional boss as players dont need to defeat it to advance in Elden Ring. Other Crucible Knights appear in the Crucible Knight and Misbegotten Warrior, Taniths Knight, Crucible Knight Ordovis, and Crucible Knight Siluria boss fights. Two  Crucible Knight spirits can also be summoned by the Spirit-Caller Snail boss of the the Roads End Catacombs. 16 Crucible Knights in total can be fought throughout the Lands Between, though two of them have evidently died and can only be fought post mortem in spirit form.',
<<<<<<< HEAD
=======
        damageType: 'Standard, Strike, Pierce, Holy',
>>>>>>> 5e9bc4f350fa6b7cc2a008018f858c80e840d59f
        whereToFind: 'Pretty much everywhere',
        weakTo: 'Fire, Lightning',
        strongVS: 'All status affects',
    },
    {
<<<<<<< HEAD
        name: 'Imp',
        img: 'https://eldenring.wiki.fextralife.com/file/Elden-Ring/fanged-imp-1-enemy-elden-ring-wiki.jpg',
        description: 'Probably the worst enemy in the game',
        whereToFind: 'Everywhere bro, literally everywhere. most commonly found in catacombs though.',
        weakTo: 'Strike',
        strongVS: 'Slash, Pierce, Fire, Lightning, Holy, Immune to all status affects'
    },
    {
        name: 'Troll',
        img: 'https://eldenring.wiki.fextralife.com/file/Elden-Ring/troll-1-elden-ring-wiki-guide.jpg',
        description: 'Trolls are Giant Humanoid Enemies in Elden Ring. Large and sturdy, these creatures are found everywhere in the Lands Between, serving as guards, knights, and even as beasts of burden.',
        whereToFind: 'Throughout the lands between',
        weakTo: 'Bleed, Sleep, strike',
        strongVS: 'Slash'
    },
    {
        name: 'Living Jar',
        img: 'https://eldenring.wiki.fextralife.com/file/Elden-Ring/living_jars_enemies_elden_ring_wiki_600px.jpg',
        description: 'Living Jar is an enemy in Elden Ring. They are animate jars that often hide out of place, usually only attacking when provoked or approached.',
        whereToFind: 'Throughout the Lands Between',
        weakTo: 'Strike',
        strongVS: 'All status affects'
    },
    {
        name: 'Lazuli Sorcerer',
        img: 'https://eldenring.wiki.fextralife.com/file/Elden-Ring/lazuli_sorcerer_enemies_elden_ring_wiki_600px.jpg',
        description: 'Lazuli Sorcerer is a Humanoid Enemy in Elden Ring. Lazuli Sorcerers are scholars of the Lazuli Conspectus at the Raya Lucaria Academy, who study Carian Sorceries - a heterodox pursuit that views the moon as equal to the stars. These pupils wield a small shield and a sword which deals magic damage.',
        whereToFind: 'Caria Manor, Raya Lucaria Academy, Selia, Town of Sorcery',
        weakTo: 'Holy, Fire',
        strongVS: 'Magic'
    },
    {
        name: 'Basilisk',
        img: 'https://eldenring.wiki.fextralife.com/file/Elden-Ring/basilisk_enemies_elden_ring_wiki_600px.jpg',
        description: 'Basilisk is an Enemy in Elden Ring. Basilisks are frog-eyed creatures found in underground areas. They are able to breathe out a black smoke that inflicts Death on players for each second they stay in it, making it important to mind your surroundings when fighting them.',
        whereToFind: 'Throughout The Lands Between',
        weakTo: 'Fire, Strike',
        strongVS: 'Holy'
    },
    {
        name: 'Wormface',
        img: 'https://eldenring.wiki.fextralife.com/file/Elden-Ring/lesser_wormface_enemies_elden_ring_wiki_600px.jpg',
        description: ' Wormface is a Enemy in Elden Ring. Lesser Wormfaces are regular enemies, usually seen around the Minor Erdtree in the Altus Plateau, and even in Farum Azula. Like their larger versions, they spew a cloud of Death Blight.',
        whereToFind: 'Altus Plateau, Crumbling Farum Azula',
        weakTo: 'Fire, Strike',
        strongVS: 'Holy'
    },
    {
        name: 'Battlemage',
        img: 'https://eldenring.wiki.fextralife.com/file/Elden-Ring/battlemage_enemies_elden_ring_wiki_600px.jpg',
        description: 'Battlemage is a mage Enemy in Elden Ring. They are mage enemies that wield a magic staff and fire ranged attacks.',
        whereToFind: 'Miquellas Haligtree',
        weakTo: 'Holy, Fire',
        strongVS: 'Magic, Slash'
    },
    {
        name: 'Albinauric',
        img: 'https://eldenring.wiki.fextralife.com/file/Elden-Ring/albinauric_1_enemies_elden_ring_wiki_600px.jpg',
        description: 'Albinaurics are Humanoid Enemies in Elden Ring. Albinaurics are said to have been artificial lifeforms that were made by humans. First-Generation Albinaurics have a human-like appearance and a flawed pair of legs, which eventually give out and sometimes even disappear. Second-Generation Albinaurics are short creatures, with smooth gray skin and dumpy heads resembling those of frogs. They use primitive weapons such as wooden clubs and makeshift shields, and they often congregate in large groups. Second-Generation Albinaurics are very agile and able to cartwheel around.',
        whereToFind: 'Throughout the Lands Between',
        weakTo: 'Bleed, Strike, Fire',
        strongVS: 'Magic, Holy'
    },
    {
        name: 'Grafted Scion',
        img: 'https://eldenring.wiki.fextralife.com/file/Elden-Ring/grafted-scion-2-elden-ring-wiki-guide.jpg',
        description: 'Grafted Scion is a Field Boss in Elden Ring. Grafted Scion is an amorphous mass of bodies that wields two swords and a greatshield, found in various locales, most prominently in Stormveil Castle.',
        whereToFind: 'Throughout the Lands Between',
        weakTo: 'Slash, Bleed, Frostbite',
        strongVS: 'Magic, Fire, Lightning, Holy'
=======
        name: '',
        img: '',
        description: '',
        damageType: '',
        whereToFind: '',
        weakTo: '',
        strongVS: ''
    },
    {
        name: '',
        img: '',
        description: '',
        damageType: '',
        whereToFind: '',
        weakTo: '',
        strongVS: ''
    },
    {
        name: '',
        img: '',
        description: '',
        damageType: '',
        whereToFind: '',
        weakTo: '',
        strongVS: ''
    },
    {
        name: '',
        img: '',
        description: '',
        damageType: '',
        whereToFind: '',
        weakTo: '',
        strongVS: ''
    },
    {
        name: '',
        img: '',
        description: '',
        damageType: '',
        whereToFind: '',
        weakTo: '',
        strongVS: ''
    },
    {
        name: '',
        img: '',
        description: '',
        damageType: '',
        whereToFind: '',
        weakTo: '',
        strongVS: ''
    },
    {
        name: '',
        img: '',
        description: '',
        damageType: '',
        whereToFind: '',
        weakTo: '',
        strongVS: ''
    },
    {
        name: '',
        img: '',
        description: '',
        damageType: '',
        whereToFind: '',
        weakTo: '',
        strongVS: ''
    },
    {
        name: '',
        img: '',
        description: '',
        damageType: '',
        whereToFind: '',
        weakTo: '',
        strongVS: ''
>>>>>>> 5e9bc4f350fa6b7cc2a008018f858c80e840d59f
    }
]
module.exports = CreaturesAndEnemies;